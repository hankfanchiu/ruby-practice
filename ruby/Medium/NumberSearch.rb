...
# search for all the numbers in the string, add them together, then return
# that final number divided by the total amount of letters in the string.
#
...
# so the answer is 2.
#
# Only single digit numbers separated by spaces will be used throughout
# the whole string (So this won't ever be the case: hello44444 world).
#
# Each string will also have at least one letter.

def NumberSearch(str)

  sum = str.scan(/[[:digit:]]/).map(&:to_i).reduce(:+)
  letter_count = str.scan(/[[:alpha:]]/).count

  (sum.to_f / letter_count).round
end
